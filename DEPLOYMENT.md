...
CI/CD（GitHub 云端 runner，ubuntu-latest）
   ...
   │  ② 通过 Cloudflare Tunnel (ssh.your-domain.example) SSH 到板子
   ▼
板子（Orange Pi Zero 3）
   │  停服务 → scp 二进制 + 前端 → 重启
   ▼
...
| 监听端口 | `8080` |
| 公网入口 | `orangepi-monitor.your-domain.example`（Cloudflare 隧道 → http://localhost:8080） |
| CI 部署入口 | `ssh.your-domain.example`（Cloudflare 隧道 → ssh://localhost:22） |
...
| 项 | 配置 | 用途 |
|---|---|---|
| 域名 | `your-domain.example` | 托管在 Cloudflare |
| 隧道 | token 模式（tunnel run --token） | 板子主动连 Cloudflare |
| Public Hostname | `orangepi-monitor.your-domain.example` → `http://localhost:8080` | 公网访问 Web |
| Public Hostname | `ssh.your-domain.example` → `ssh://localhost:22` | CI/CD 部署 SSH 入口（两项目共用） |
...
### 6.2 Cloudflare 配置
1. 域名托管在 Cloudflare（`your-domain.example`）
2. 板子安装 cloudflared，用 token 接入隧道
3. 加 Public Hostname：
   - `orangepi-monitor.your-domain.example` → `http://localhost:8080`
   - `ssh.your-domain.example` → `ssh://localhost:22`（两项目共用）
4. Access → Service Auth 创建 Service Token（记下 Client ID/Secret）
5. Access → 为 `ssh.your-domain.example` 配置 `ci-deploy` 策略（Service Auth）
